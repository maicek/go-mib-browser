...
BUILD_FLAGS_WINDOWS := -a -trimpath -ldflags="-s -w -X main.embedPath=dist -extldflags '-static'" -gcflags="all=-l=4" -tags=netgo
...
build-windows-x86:
	CGO_ENABLED=1 CC=x86_64-w64-mingw32-gcc CXX=x86_64-w64-mingw32-g++ GOOS=windows GOARCH=amd64 go build $(BUILD_FLAGS_WINDOWS) -o $(BINARY_NAME)-windows-amd64.exe $(SRC_DIR)
